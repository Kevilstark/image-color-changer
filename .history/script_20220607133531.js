...
$(document).ready(() => {
  $('button.execute').click(() => {
    ...
    const triedtime = $('input.time-tried').val();
    if (rgb.length !== 3) {
    ...
    let resultTitleVal = document.createTextNode('runtime');

    let para = document.createElement("button");
    para.setAttribute('id', 'clickResult')
    let node = document.createTextNode("See all result");
    para.appendChild(node);
    elementAdd.appendChild(para);
    let resultTable = document.createElement('table');
    resultTable.setAttribute('id', 'all-result')
    elementAdd.appendChild(resultTable);
    ...
    for(var i=0;i<triedtime;i++) {
        ...
        cell0.innerHTML = i+1;
        cell1.innerHTML = resultChange.filter;
        cell2.innerHTML = resultChange.loss.toFixed(3);

        if(resultChange.loss < min){
          min = resultChange.loss;
          result = resultChange;
        }
    ...
    $('.lossDetail').html(`Loss: ${result.loss.toFixed(3)}. <b>${lossMsg}</b>`);
    $("#all-result").hide();
    $('#clickResult').click(() => {
      if ($("#all-result").is(":hidden"))
        $("#all-result").show();
        else $("#all-result").hide();
    });
